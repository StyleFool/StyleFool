...
def calculate_color(style_folder, save_path, maxColor=7):
    ...
    for style in styles:
        ...
        try:
            ...
            if black_edge_flag:
                a = 1 / 0 # call except
            themes = [testMMCQ(pixDatas, maxColor)] # theme colors
            style_top_list = themes[0][0]
            style_top_hsv = RGB2HSV_batch(style_top_list).tolist()
            if style_top_hsv[0][2] < 0.138: # black will not become the first theme color
                a = 1 / 0 # call except
        except:
            themes = [[[]]]
            style_top_list = []
...
def check_danger_label(styles_themes_info, styles_top_hsv_info, styles, class_info, num=101):
    # prevent that all videos of a label all have all non-semantic black areas in a row or black theme color,
...
def select_style_target(style_folder, video_path, class_info, attack_id, random_seed=20008, miu=100000):
    ...
    with open("batch_command_target.sh", "w") as f:
        for id in slice:
            ...
            # prevent that the attacked video has all non-semantic black areas in a row or black theme color,
            # especially in UCF-101 dataset,
